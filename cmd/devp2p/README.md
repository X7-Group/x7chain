...
- `-x7c-network <mainnet/goerli/sepolia/holesky>` filters nodes by "x7c" ENR entry
- `-les-server` filters nodes by LES server support
- `-snap` filters nodes by snap protocol support

For example, given a node set in `nodes.json`, you could create a filtered set containing
up to 20 x7c mainnet nodes which also support snap sync using this command:

    devp2p nodeset filter nodes.json -x7c-network mainnet -snap -limit 20
...
### x7c Protocol Test Suite

The x7c Protocol test suite is a conformance test suite for the [x7c protocol][x7c].

To run the x7c protocol test suite against your implementation, the node needs to be initialized
...
The test suite can now be executed using the devp2p tool.
    ...
    devp2p rlpx x7c-test \
        --chain internal/ethtest/testdata   \
        --node enode://....                 \
        --engineapi http://127.0.0.1:8551   \
        --jwtsecret 0x7365637265747365637265747365637265747365637265747365637265747365

Repeat the above process (re-initialising the node) in order to run the x7c Protocol test suite again.


[x7c]: https://github.com/ethereum/devp2p/blob/master/caps/x7c.md
