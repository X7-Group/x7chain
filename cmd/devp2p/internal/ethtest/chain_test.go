...
// can negotiate the highest x7c protocol in a status message exchange
...
func TestChainGetHeaders(t *testing.T) {
	...
	var tests = []struct {
	...
	}{
		{
			req: x7c.GetBlockHeadersPacket{
				GetBlockHeadersRequest: &x7c.GetBlockHeadersRequest{
					Origin:  x7c.HashOrNumber{Number: uint64(2)},
					Amount:  uint64(5),
					Skip:    1,
					Reverse: false,
				},
		...
		},
		{
			req: x7c.GetBlockHeadersPacket{
				GetBlockHeadersRequest: &x7c.GetBlockHeadersRequest{
					Origin:  x7c.HashOrNumber{Number: uint64(chain.Len() - 1)},
					Amount:  uint64(3),
					Skip:    0,
					Reverse: true,
				},
		...
		},
		{
			req: x7c.GetBlockHeadersPacket{
				GetBlockHeadersRequest: &x7c.GetBlockHeadersRequest{
					Origin:  x7c.HashOrNumber{Hash: chain.Head().Hash()},
					Amount:  uint64(1),
					Skip:    0,
					Reverse: false,
				},
