...
import (
	...
	"github.com/X7-Group/x7chain/consensus/misc/eip4844"
	"github.com/X7-Group/x7chain/core/types"
	"github.com/X7-Group/x7chain/crypto"
	"github.com/X7-Group/x7chain/crypto/kzg4844"
	"github.com/X7-Group/x7chain/eth/protocols/x7c"
	"github.com/X7-Group/x7chain/internal/utesting"
...
// NewSuite creates and returns a new x7c-test suite that can
...
// message with it on the x7c protocol.
...
// TestGetBlockHeaders tests whether the given node can respond to an x7c
...
func (s *Suite) TestGetBlockHeaders(t *utesting.T) {
	...
	req := &x7c.GetBlockHeadersPacket{
		RequestId: 33,
		GetBlockHeadersRequest: &x7c.GetBlockHeadersRequest{
			Origin:  x7c.HashOrNumber{Hash: s.chain.blocks[1].Hash()},
			Amount:  2,
			Skip:    1,
			Reverse: false,
		},
	...
	if err := conn.Write(ethProto, x7c.GetBlockHeadersMsg, req); err != nil {
		t.Fatalf("could not write to connection: %v", err)
	}
	headers := new(x7c.BlockHeadersPacket)
	if err := conn.ReadMsg(ethProto, x7c.BlockHeadersMsg, &headers); err != nil {
...
func (s *Suite) TestSimultaneousRequests(t *utesting.T) {
	...
	req1 := &x7c.GetBlockHeadersPacket{
	...
	req2 := &x7c.GetBlockHeadersPacket{
	...
	// Send both requests.
	if err := conn.Write(ethProto, x7c.GetBlockHeadersMsg, req1); err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}
	if err := conn.Write(ethProto, x7c.GetBlockHeadersMsg, req2); err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}

	// Wait for responses.
	headers1 := new(x7c.BlockHeadersPacket)
	if err := conn.ReadMsg(ethProto, x7c.BlockHeadersMsg, &headers1); err != nil {
		t.Fatalf("error reading block headers msg: %v", err)
	}
	if got, want := headers1.RequestId, req1.RequestId; got != want {
		t.Fatalf("unexpected request id in response: got %d, want %d", got, want)
	}
	headers2 := new(x7c.BlockHeadersPacket)
	if err := conn.ReadMsg(ethProto, x7c.BlockHeadersMsg, &headers2); err != nil {
		t.Fatalf("error reading block headers msg: %v", err)
	}
...
func (s *Suite) TestSameRequestID(t *utesting.T) {
	...
	reqID := uint64(1234)
	request1 := &x7c.GetBlockHeadersPacket{
		RequestId: reqID,
		GetBlockHeadersRequest: &x7c.GetBlockHeadersRequest{
			Origin: x7c.HashOrNumber{
				Number: 1,
	...
	request2 := &x7c.GetBlockHeadersPacket{
		RequestId: reqID,
		GetBlockHeadersRequest: &x7c.GetBlockHeadersRequest{
			Origin: x7c.HashOrNumber{
				Number: 33,
	...
	// Send the requests.
	if err = conn.Write(ethProto, x7c.GetBlockHeadersMsg, request1); err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}
	if err = conn.Write(ethProto, x7c.GetBlockHeadersMsg, request2); err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}
...
func (s *Suite) TestZeroRequestID(t *utesting.T) {
	...
	req := &x7c.GetBlockHeadersPacket{
		GetBlockHeadersRequest: &x7c.GetBlockHeadersRequest{
			Origin: x7c.HashOrNumber{Number: 0},
			Amount: 2,
		},
	}
	// Read headers response.
...
func (s *Suite) TestGetBlockBodies(t *utesting.T) {
	...
	req := &x7c.GetBlockBodiesPacket{
		RequestId: 55,
		GetBlockBodiesRequest: x7c.GetBlockBodiesRequest{
			s.chain.blocks[54].Hash(),
			s.chain.blocks[75].Hash(),
		},
	}
	if err := conn.Write(ethProto, x7c.GetBlockBodiesMsg, req); err != nil {
		t.Fatalf("could not write to connection: %v", err)
	}
	// Wait for response.
	resp := new(x7c.BlockBodiesPacket)
	if err := conn.ReadMsg(ethProto, x7c.BlockBodiesMsg, &resp); err != nil {
...
func (s *Suite) TestMaliciousHandshake(t *utesting.T) {
	...
	var (
		...
		version = x7c.ProtocolVersions[0]
	)
...
func (s *Suite) TestLargeTxRequest(t *utesting.T) {
	...
	req := &x7c.GetPooledTransactionsPacket{
		RequestId:                    1234,
		GetPooledTransactionsRequest: hashes,
	}
	if err = conn.Write(ethProto, x7c.GetPooledTransactionsMsg, req); err != nil {
		t.Fatalf("could not write to conn: %v", err)
	}
	// Check that all received transactions match those that were sent to node.
	msg := new(x7c.PooledTransactionsPacket)
	if err := conn.ReadMsg(ethProto, x7c.PooledTransactionsMsg, &msg); err != nil {
		t.Fatalf("error reading from connection: %v", err)
	}
...
func (s *Suite) TestNewPooledTxs(t *utesting.T) {
	...
	// Send announcement.
	ann := x7c.NewPooledTransactionHashesPacket68{Types: txTypes, Sizes: sizes, Hashes: hashes}
	err = conn.Write(ethProto, x7c.NewPooledTransactionHashesMsg, ann)
	if err != nil {
		t.Fatalf("failed to write to connection: %v", err)
	}
	...
	for {
		...
		if err != nil {
			t.Fatalf("failed to read x7c msg: %v", err)
		}
		switch msg := msg.(type) {
		case *x7c.GetPooledTransactionsPacket:
...
func (s *Suite) TestBlobViolations(t *utesting.T) {
	...
	for _, test := range []struct {
		ann  x7c.NewPooledTransactionHashesPacket68
		resp x7c.PooledTransactionsResponse
	}{
		// Invalid tx size.
		{
			ann: x7c.NewPooledTransactionHashesPacket68{
			...
			resp: x7c.PooledTransactionsResponse(t1),
		},
		// Wrong tx type.
		{
			ann: x7c.NewPooledTransactionHashesPacket68{
			...
			resp: x7c.PooledTransactionsResponse(t2),
		},
	} {
		conn, err := s.dial()
		if err != nil {
		...
		if err := conn.Write(ethProto, x7c.NewPooledTransactionHashesMsg, test.ann); err != nil {
			t.Fatalf("sending announcement failed: %v", err)
		}
		req := new(x7c.GetPooledTransactionsPacket)
		if err := conn.ReadMsg(ethProto, x7c.GetPooledTransactionsMsg, req); err != nil {
			t.Fatalf("reading pooled tx request failed: %v", err)
		}
		resp := x7c.PooledTransactionsPacket{RequestId: req.RequestId, PooledTransactionsResponse: test.resp}
		if err := conn.Write(ethProto, x7c.PooledTransactionsMsg, resp); err != nil {
