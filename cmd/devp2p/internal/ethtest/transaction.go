...
func (s *Suite) sendTxs(txs []*types.Transaction) error {
	...
	if err = sendConn.Write(ethProto, x7c.TransactionsMsg, x7c.TransactionsPacket(txs)); err != nil {
	...
	for time.Now().Before(end) {
		...
		default:
			return fmt.Errorf("unexpected x7c wire msg: %s", pretty.Sdump(msg))
		}
...
func (s *Suite) sendInvalidTxs(txs []*types.Transaction) error {
	...
	if err = sendConn.Write(ethProto, x7c.TransactionsMsg, txs); err != nil {
	...
	for {
		...
		default:
			return fmt.Errorf("unexpected x7c message: %v", pretty.Sdump(msg))
		}
	}
}
