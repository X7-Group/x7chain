...
import (
	...
	"github.com/X7-Group/x7chain/accounts/external"
	"github.com/X7-Group/x7chain/accounts/keystore"
	"github.com/X7-Group/x7chain/accounts/scwallet"
	"github.com/X7-Group/x7chain/accounts/usbwallet"
	"github.com/X7-Group/x7chain/cmd/utils"
	...
	"github.com/X7-Group/x7chain/internal/version"
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "x7c")
	cfg.WSModules = append(cfg.WSModules, "x7c")
...
func makeConfigNode(ctx *cli.Context) (*node.Node, gethConfig) {
	...
	utils.SetEthConfig(ctx, stack, &cfg.x7c)
...
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if ctx.IsSet(utils.OverrideCancun.Name) {
		...
		cfg.x7c.OverrideCancun = &v
	}
	if ctx.IsSet(utils.OverrideVerkle.Name) {
		v := ctx.Uint64(utils.OverrideVerkle.Name)
		cfg.x7c.OverrideVerkle = &v
	}
	backend, x7c := utils.RegisterEthService(stack, &cfg.x7c)

	// Create gauge with geth system and build information
	if x7c != nil { // The 'x7c' backend may be nil in light mode
		var protos []string
		for _, p := range x7c.Protocols() {
	...
	filterSystem := utils.RegisterFilterAPI(stack, backend, &cfg.x7c)
	...
	if ctx.IsSet(utils.SyncTargetFlag.Name) {
		...
		utils.RegisterFullSyncTester(stack, x7c, common.BytesToHash(hex))
	}
	// Start the dev mode if requested, or launch the engine API for
	// interacting with external consensus client.
	if ctx.IsSet(utils.DeveloperFlag.Name) {
		simBeacon, err := catalyst.NewSimulatedBeacon(ctx.Uint64(utils.DeveloperPeriodFlag.Name), x7c)
		if err != nil {
		...
		err := catalyst.Register(stack, x7c)
		if err != nil {
