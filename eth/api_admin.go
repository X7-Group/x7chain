...
func NewAdminAPI(x7c *Ethereum) *AdminAPI {
	return &AdminAPI{x7c: x7c}
}
...
func (api *AdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.x7c.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.x7c.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.x7c.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *AdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.x7c.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.x7c.BlockChain().InsertChain(blocks); err != nil {
