...
func NewDebugAPI(x7c *Ethereum) *DebugAPI {
	return &DebugAPI{x7c: x7c}
}
...
func (api *DebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.x7c.miner.Pending()
		if stateDb == nil {
	...
	case rpc.LatestBlockNumber:
		header = api.x7c.blockchain.CurrentBlock()
	case rpc.FinalizedBlockNumber:
		header = api.x7c.blockchain.CurrentFinalBlock()
	case rpc.SafeBlockNumber:
		header = api.x7c.blockchain.CurrentSafeBlock()
	default:
		block := api.x7c.blockchain.GetBlockByNumber(uint64(blockNr))
		if block == nil {
	...
	stateDb, err := api.x7c.BlockChain().StateAt(header.Root)
	if err != nil {
...
func (api *DebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.x7c.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *DebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		blocks  = rawdb.ReadAllBadBlocks(api.x7c.chainDb)
	...
	for _, block := range blocks {
		...
		blockJSON = ethapi.RPCMarshalBlock(block, true, true, api.x7c.APIBackend.ChainConfig())
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			case rpc.SafeBlockNumber:
				header = api.x7c.blockchain.CurrentSafeBlock()
			default:
				block := api.x7c.blockchain.GetBlockByNumber(uint64(number))
				if block == nil {
		...
		stateDb, err = api.x7c.BlockChain().StateAt(block.Root())
		if err != nil {
			return state.Dump{}, err
		}
	} else {
...
func (api *DebugAPI) StorageRangeAt(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	...
	block, err := api.x7c.APIBackend.BlockByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	_, _, statedb, release, err := api.x7c.stateAtTransaction(ctx, block, txIndex, 0)
	if err != nil {
		return StorageRangeResult{}, err
	}
	defer release()
...
func (api *DebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.x7c.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.x7c.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.x7c.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *DebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.x7c.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.x7c.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *DebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.x7c.BlockChain().TrieDB()
...
func (api *DebugAPI) GetAccessibleState(from, to rpc.BlockNumber) (uint64, error) {
	if api.x7c.blockchain.TrieDB().Scheme() == rawdb.PathScheme {
	...
	var resolveNum = func(num rpc.BlockNumber) (uint64, error) {
		...
		if num.Int64() < 0 {
			block := api.x7c.blockchain.CurrentBlock()
			if block == nil {
	...
	for i := int64(start); i != int64(end); i += delta {
		...
		h := api.x7c.BlockChain().GetHeaderByNumber(uint64(i))
		if h == nil {
			return 0, fmt.Errorf("missing header %d", i)
		}
		if ok, _ := api.x7c.ChainDb().Has(h.Root[:]); ok {
			return uint64(i), nil
		}
	}
...
func (api *DebugAPI) SetTrieFlushInterval(interval string) error {
	...
	api.x7c.blockchain.SetTrieFlushInterval(t)
	return nil
}
...
func (api *DebugAPI) GetTrieFlushInterval() (string, error) {
	...
	return api.x7c.blockchain.GetTrieFlushInterval().String(), nil
}
