...
import (
	...
	"github.com/X7-Group/x7chain/beacon/engine"
	"github.com/X7-Group/x7chain/common"
	"github.com/X7-Group/x7chain/common/hexutil"
	...
	"github.com/X7-Group/x7chain/node"
	"github.com/X7-Group/x7chain/rpc"
	"github.com/X7-Group/x7chain/eth"
)

// Register adds the engine API to the full node.
func Register(stack *node.Node, backend *x7c.Ethereum) error {
	log.Warn("Engine API enabled", "protocol", "x7c")
...
func NewConsensusAPI(x7c *x7c.Ethereum) *ConsensusAPI {
	api := newConsensusAPIWithoutHeartbeat(x7c)
	go api.heartbeat()
	return api
}

// newConsensusAPIWithoutHeartbeat creates a new consensus api for the SimulatedBeacon Node.
func newConsensusAPIWithoutHeartbeat(x7c *x7c.Ethereum) *ConsensusAPI {
	if x7c.BlockChain().Config().TerminalTotalDifficulty == nil {
	...
	x7c.Downloader().SetBadBlockCallback(api.setInvalidAncestor)
	return api
}
...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if payloadAttributes != nil {
		...
		if api.x7c.BlockChain().Config().IsShanghai(api.x7c.BlockChain().Config().LondonBlock, payloadAttributes.Timestamp) {
...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	...
	// reason.
	block := api.x7c.BlockChain().GetBlockByHash(update.HeadBlockHash)
	if block == nil {
		...
		// the future to resolve from the `x7c` network, but it's an unexpected case
		...
		if merger := api.x7c.Merger(); !merger.TDDReached() {
			merger.ReachTTD()
			api.x7c.Downloader().Cancel()
		}
		...
		if err := api.x7c.Downloader().BeaconSync(api.x7c.SyncMode(), header, finalized); err != nil {
	...
	if block.Difficulty().BitLen() > 0 || block.NumberU64() == 0 {
		var (
			td  = api.x7c.BlockChain().GetTd(update.HeadBlockHash, block.NumberU64())
			ptd = api.x7c.BlockChain().GetTd(block.ParentHash(), block.NumberU64()-1)
			ttd = api.x7c.BlockChain().Config().TerminalTotalDifficulty
		)
	...
	if rawdb.ReadCanonicalHash(api.x7c.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		// Block is not canonical, set head.
		if latestValid, err := api.x7c.BlockChain().SetCanonical(block); err != nil {
			return engine.ForkChoiceResponse{PayloadStatus: engine.PayloadStatusV1{Status: engine.INVALID, LatestValidHash: &latestValid}}, err
		}
	} else if api.x7c.BlockChain().CurrentBlock().Hash() == update.HeadBlockHash {
		...
		log.Info("Ignoring beacon update to old head", "number", block.NumberU64(), "hash", update.HeadBlockHash, "age", common.PrettyAge(time.Unix(int64(block.Time()), 0)), "have", api.x7c.BlockChain().CurrentBlock().Number)
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		if merger := api.x7c.Merger(); !merger.PoSFinalized() {
			merger.FinalizePoS()
		}
		// If the finalized block is not in our canonical tree, somethings wrong
		finalBlock := api.x7c.BlockChain().GetBlockByHash(update.FinalizedBlockHash)
		if finalBlock == nil {
			log.Warn("Final block not available in database", "hash", update.FinalizedBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not available in database"))
		} else if rawdb.ReadCanonicalHash(api.x7c.ChainDb(), finalBlock.NumberU64()) != update.FinalizedBlockHash {
			log.Warn("Final block not in canonical chain", "number", block.NumberU64(), "hash", update.HeadBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not in canonical chain"))
		}
		// Set the finalized block
		api.x7c.BlockChain().SetFinalized(finalBlock.Header())
	}
	// Check if the safe block hash is in our canonical tree, if not somethings wrong
	if update.SafeBlockHash != (common.Hash{}) {
		safeBlock := api.x7c.BlockChain().GetBlockByHash(update.SafeBlockHash)
		if safeBlock == nil {
			log.Warn("Safe block not available in database")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not available in database"))
		}
		if rawdb.ReadCanonicalHash(api.x7c.ChainDb(), safeBlock.NumberU64()) != update.SafeBlockHash {
			log.Warn("Safe block not in canonical chain")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not in canonical chain"))
		}
		// Set the safe block
		api.x7c.BlockChain().SetSafe(safeBlock.Header())
	}
	...
	if payloadAttributes != nil {
		...
		payload, err := api.x7c.Miner().BuildPayload(args)
		if err != nil {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if config.TerminalBlockHash != (common.Hash{}) {
		if hash := api.x7c.BlockChain().GetCanonicalHash(uint64(config.TerminalBlockNumber)); hash == config.TerminalBlockHash {
...
func (api *ConsensusAPI) NewPayloadV2(params engine.ExecutableData) (engine.PayloadStatusV1, error) {
	if api.x7c.BlockChain().Config().IsShanghai(new(big.Int).SetUint64(params.Number), params.Timestamp) {
		if params.Withdrawals == nil {
	...
	if api.x7c.BlockChain().Config().IsCancun(new(big.Int).SetUint64(params.Number), params.Timestamp) {
...
func (api *ConsensusAPI) newPayload(params engine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash) (engine.PayloadStatusV1, error) {
	...
	//      api.x7c.BlockChain().InsertBlockWithoutSetHead, where it is blocked on
	...
	if block := api.x7c.BlockChain().GetBlockByHash(params.BlockHash); block != nil {
	...
	parent := api.x7c.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		ptd  = api.x7c.BlockChain().GetTd(parent.Hash(), parent.NumberU64())
		ttd  = api.x7c.BlockChain().Config().TerminalTotalDifficulty
		gptd = api.x7c.BlockChain().GetTd(parent.ParentHash(), parent.NumberU64()-1)
	)
	if ptd.Cmp(ttd) < 0 {
	...
	if api.x7c.SyncMode() != downloader.FullSync {
		return api.delayPayloadImport(block)
	}
	if !api.x7c.BlockChain().HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
		api.remoteBlocks.put(block.Hash(), block.Header())
		log.Warn("State not available, ignoring new payload")
		return engine.PayloadStatusV1{Status: engine.ACCEPTED}, nil
	}
	log.Trace("Inserting block without sethead", "hash", block.Hash(), "number", block.Number)
	if err := api.x7c.BlockChain().InsertBlockWithoutSetHead(block); err != nil {
...
func (api *ConsensusAPI) delayPayloadImport(block *types.Block) (engine.PayloadStatusV1, error) {
	...
	err := api.x7c.Downloader().BeaconExtend(api.x7c.SyncMode(), block.Header())
	if err == nil {
...
func (api *ConsensusAPI) checkInvalidAncestor(check common.Hash, head common.Hash) *engine.PayloadStatusV1 {
	...
	if header := api.x7c.BlockChain().GetHeader(invalid.ParentHash, invalid.Number.Uint64()-1); header != nil && header.Difficulty.Sign() != 0 {
...
func (api *ConsensusAPI) heartbeat() {
	...
	if api.x7c.BlockChain().Config().TerminalTotalDifficulty == nil {
		return
	}
	...
	for {
		...
		if api.x7c.BlockChain().Config().TerminalTotalDifficultyPassed || api.x7c.Merger().TDDReached() {
...
func (api *ConsensusAPI) GetPayloadBodiesByHashV1(hashes []common.Hash) []*engine.ExecutionPayloadBodyV1 {
	...
	for i, hash := range hashes {
		block := api.x7c.BlockChain().GetBlockByHash(hash)
...
func (api *ConsensusAPI) GetPayloadBodiesByRangeV1(start, count hexutil.Uint64) ([]*engine.ExecutionPayloadBodyV1, error) {
	...
	current := api.x7c.BlockChain().CurrentBlock().Number.Uint64()
	...
	for i := uint64(start); i <= last; i++ {
		block := api.x7c.BlockChain().GetBlockByNumber(i)
