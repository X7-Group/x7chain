...
import (
	...
	beaconConsensus "github.com/X7-Group/x7chain/consensus/beacon"
...
func TestEth2AssembleBlockWithAnotherBlocksTxs(t *testing.T) {
	...
	api.x7c.TxPool().Add(txs, false, true)
	blockParams := engine.PayloadAttributes{
...
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *x7c.Ethereum) {
	t.Helper()
	...
	ethservice, err := x7c.New(n, ethcfg)
	if err != nil {
		t.Fatal("can't create x7c service:", err)
	}
...
func setupBlocks(t *testing.T, ethservice *x7c.Ethereum, n int, parent *types.Header, callback func(parent *types.Header), withdrawals [][]*types.Withdrawal) []*types.Header {
	api := NewConsensusAPI(ethservice)
...
func setupBodies(t *testing.T) (*node.Node, *x7c.Ethereum, []*types.Block) {
	genesis, blocks := generateMergeChain(10, true)
...
func TestGetBlockBodiesByHash(t *testing.T) {
	node, x7c, blocks := setupBodies(t)
	api := NewConsensusAPI(x7c)
	defer node.Close()
	...
	tests := []struct {
		...
		// First pow block
		{
			results: []*types.Body{x7c.BlockChain().GetBlockByNumber(0).Body()},
			hashes:  []common.Hash{x7c.BlockChain().GetBlockByNumber(0).Hash()},
		},
		// Last pow block
		{
