...
func NewSimulatedBeacon(period uint64, x7c *x7c.Ethereum) (*SimulatedBeacon, error) {
	block := x7c.BlockChain().CurrentBlock()
	current := engine.ForkchoiceStateV1{
		HeadBlockHash:      block.Hash(),
		SafeBlockHash:      block.Hash(),
		FinalizedBlockHash: block.Hash(),
	}
	engineAPI := newConsensusAPIWithoutHeartbeat(x7c)
...
func (c *SimulatedBeacon) sealBlock(withdrawals []*types.Withdrawal, timestamp uint64) error {
	...
	if header := c.x7c.BlockChain().CurrentBlock(); c.curForkchoiceState.HeadBlockHash != header.Hash() {
...
func (c *SimulatedBeacon) finalizedBlockHash(number uint64) *common.Hash {
	...
	if finalizedBlock := c.x7c.BlockChain().GetBlockByNumber(finalizedNumber); finalizedBlock != nil {
...
func (c *SimulatedBeacon) Commit() common.Hash {
	...
	return c.x7c.BlockChain().CurrentBlock().Hash()
}
...
func (c *SimulatedBeacon) Rollback() {
	...
	c.x7c.TxPool().SetGasTip(maxUint256)
	// Set the gas tip back to accept new transactions
	// TODO (Marius van der Wijden): set gas tip to parameter passed by config
	c.x7c.TxPool().SetGasTip(big.NewInt(params.GWei))
}

// Fork sets the head to the provided hash.
func (c *SimulatedBeacon) Fork(parentHash common.Hash) error {
	if len(c.x7c.TxPool().Pending(false)) != 0 {
		return errors.New("pending block dirty")
	}
	parent := c.x7c.BlockChain().GetBlockByHash(parentHash)
	if parent == nil {
		return errors.New("parent not found")
	}
	return c.x7c.BlockChain().SetHead(parent.NumberU64())
}
...
func (c *SimulatedBeacon) AdjustTime(adjustment time.Duration) error {
	...
	parent := c.x7c.BlockChain().CurrentBlock()
	if parent == nil {
		return errors.New("parent not found")
	}
	withdrawals := c.withdrawals.gatherPending(10)
