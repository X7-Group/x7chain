...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [x7c/62] Maximum number of headers to queue for import (DOS protection)
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td, ttd *big.Int, beaconMode bool) (err error) {
	...
	if !beaconMode {
		log.Debug("Synchronising with the network", "peer", p.id, "x7c", p.version, "head", hash, "td", td, "mode", mode)
	} else {
