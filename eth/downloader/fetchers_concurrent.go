...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *x7c.Response) (*x7c.Request, error)
	...
	deliver(peer *peerConnection, packet *x7c.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	responses := make(chan *x7c.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*x7c.Request)
	defer func() {
	...
	ordering := make(map[*x7c.Request]int)
	timeouts := prque.New[int64, *x7c.Request](func(data *x7c.Request, index int) {
		ordering[data] = index
	})
	...
	stales := make(map[string]*x7c.Request)
	defer func() {
		// Abort all requests on sync cycle cancellation. The requests may still
		// be fulfilled by the remote side, but the dispatcher will not wait to
		// deliver them since nobody's going to be listening.
