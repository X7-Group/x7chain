...
import (
	...
	"github.com/X7-Group/x7chain/p2p/msgrate"
)
...
type peerConnection struct {
	...
	version uint       // x7c protocol version number to switch strategies
...
type Peer interface {
	...
	RequestHeadersByHash(common.Hash, int, int, bool, chan *x7c.Response) (*x7c.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *x7c.Response) (*x7c.Request, error)

	RequestBodies([]common.Hash, chan *x7c.Response) (*x7c.Request, error)
	RequestReceipts([]common.Hash, chan *x7c.Response) (*x7c.Request, error)
}
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(x7c.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(x7c.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(x7c.ReceiptsMsg, elapsed, delivered)
}

// HeaderCapacity retrieves the peer's header download allowance based on its
// previously discovered throughput.
func (p *peerConnection) HeaderCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(x7c.BlockHeadersMsg, targetRTT)
...
func (p *peerConnection) BodyCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(x7c.BlockBodiesMsg, targetRTT)
...
func (p *peerConnection) ReceiptCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(x7c.ReceiptsMsg, targetRTT)
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		rates: msgrate.NewTrackers(log.New("proto", "x7c")),
	}
}
