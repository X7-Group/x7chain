...
func (p *skeletonTestPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *x7c.Response) (*x7c.Request, error) {
	...
	if headers == nil {
		...
		if len(p.headers) > int(origin) { // Don't serve headers if we're missing the origin
			for i := 0; i < amount; i++ {
				...
				// attack by sending/receiving those over x7c.
...
func (p *skeletonTestPeer) RequestHeadersByHash(common.Hash, int, int, bool, chan *x7c.Response) (*x7c.Request, error) {
	panic("skeleton sync must not request headers by hash")
}

func (p *skeletonTestPeer) RequestBodies([]common.Hash, chan *x7c.Response) (*x7c.Request, error) {
	panic("skeleton sync must not request block bodies")
}

func (p *skeletonTestPeer) RequestReceipts([]common.Hash, chan *x7c.Response) (*x7c.Request, error) {
...
func TestSkeletonSyncRetrievals(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for _, peer := range tt.peers {
			peerset.Register(newPeerConnection(peer.id, x7c.ETH67, peer, log.New("id", peer.id)))
		}
		...
		if tt.newPeer != nil {
			if err := peerset.Register(newPeerConnection(tt.newPeer.id, x7c.ETH67, tt.newPeer, log.New("id", tt.newPeer.id))); err != nil {
