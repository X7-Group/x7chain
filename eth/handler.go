...
import (
	...
	"github.com/X7-Group/x7chain/eth/fetcher"
...
// runEthPeer registers an x7c peer into the joint x7c/snap peerset, adds it to
// various subsystems and starts handling messages.
func (h *handler) runEthPeer(peer *x7c.Peer, handler x7c.Handler) error {
	if !h.incHandlers() {
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in x7c syncer", "err", err)
		return err
	}
	if snap != nil {
	...
	for number, hash := range h.requiredBlocks {
		...
		go func(number uint64, hash common.Hash, req *x7c.Request) {
			...
			select {
			case res := <-resCh:
				headers := ([]*types.Header)(*res.Res.(*x7c.BlockHeadersRequest))
				if len(headers) == 0 {
...
// runSnapExtension registers a `snap` peer into the joint x7c/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `x7c`, all subsystem registrations and lifecycle management will be done by
// the main `x7c` handler to prevent strange races.
...
func (h *handler) unregisterPeer(id string) {
	...
	// Remove the `x7c` peer if it exists
