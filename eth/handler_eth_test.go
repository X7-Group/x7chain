...
import (
	...
	"github.com/X7-Group/x7chain/core/forkid"
...
// on the `x7c` protocol and convert them into a more easily testable form.
...
func (h *testEthHandler) TxPool() x7c.TxPool                   { panic("no backing tx pool") }
func (h *testEthHandler) AcceptTxs() bool                      { return true }
func (h *testEthHandler) RunPeer(*x7c.Peer, x7c.Handler) error { panic("not used in tests") }
func (h *testEthHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testEthHandler) Handle(peer *x7c.Peer, packet x7c.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected x7c packet type in tests: %T", packet))
	}
}

// Tests that peers are correctly accepted (or rejected) based on the advertised
// fork IDs in the protocol handshake.
func TestForkIDSplit67(t *testing.T) { testForkIDSplit(t, x7c.ETH67) }
func TestForkIDSplit68(t *testing.T) { testForkIDSplit(t, x7c.ETH68) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	peerNoFork := x7c.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := x7c.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- ethNoFork.runEthPeer(peerProFork, func(peer *x7c.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- ethProFork.runEthPeer(peerNoFork, func(peer *x7c.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = x7c.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = x7c.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions67(t *testing.T) { testRecvTransactions(t, x7c.ETH67) }
func TestRecvTransactions68(t *testing.T) { testRecvTransactions(t, x7c.ETH68) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := x7c.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := x7c.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runEthPeer(sink, func(peer *x7c.Peer) error {
		return x7c.Handle((*ethHandler)(handler.handler), peer)
	})
...
func TestSendTransactions67(t *testing.T) { testSendTransactions(t, x7c.ETH67) }
func TestSendTransactions68(t *testing.T) { testSendTransactions(t, x7c.ETH68) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runEthPeer(src, func(peer *x7c.Peer) error {
		return x7c.Handle((*ethHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 67, 68:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post x7c/66")
			}
...
func TestTransactionPropagation67(t *testing.T) { testTransactionPropagation(t, x7c.ETH67) }
func TestTransactionPropagation68(t *testing.T) { testTransactionPropagation(t, x7c.ETH68) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := x7c.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i + 1)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := x7c.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runEthPeer(sourcePeer, func(peer *x7c.Peer) error {
			return x7c.Handle((*ethHandler)(source.handler), peer)
		})
		go sink.handler.runEthPeer(sinkPeer, func(peer *x7c.Peer) error {
			return x7c.Handle((*ethHandler)(sink.handler), peer)
		})
	}
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := x7c.NewPeer(x7c.ETH67, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := x7c.NewPeer(x7c.ETH67, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock67(t *testing.T) { testBroadcastMalformedBlock(t, x7c.ETH67) }
func TestBroadcastMalformedBlock68(t *testing.T) { testBroadcastMalformedBlock(t, x7c.ETH68) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := x7c.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := x7c.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runEthPeer(src, func(peer *x7c.Peer) error {
		return x7c.Handle((*ethHandler)(source.handler), peer)
	})
