...
var (
	...
	// snap protocol without advertising the x7c main protocol.
	errSnapWithoutEth = errors.New("peer connected on snap without compatible x7c support")
)

// peerSet represents the collection of active peers currently participating in
// the `x7c` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*ethPeer // Peers connected on the `x7c` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `x7c` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `x7c`
...
// registerSnapExtension unblocks an already connected `x7c` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `x7c` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `x7c` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `x7c`
	if !peer.RunningCap(x7c.ProtocolName, x7c.ProtocolVersions) {
	...
	// Inject the peer into an `x7c` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *x7c.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `x7c` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *x7c.Peer, ext *snap.Peer) error {
...
// len returns if the current number of `x7c` peers in the set. Since the `snap`
// peers are tied to the existence of an `x7c` connection, that will always be a
// subset of `x7c`.
...
func (ps *peerSet) peerWithHighestTD() *x7c.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
