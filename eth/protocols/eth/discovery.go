...
// enrEntry is the ENR entry which advertises `x7c` protocol on the discovery.
...
// StartENRUpdater starts the `x7c` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `x7c` ENR entry based on the current state of the chain.
