...
import (
	...
	"github.com/X7-Group/x7chain/p2p/enr"
...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `x7c` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `x7c`.
func MakeProtocols(backend Backend, network uint64, dnsdisc enode.Iterator) []p2p.Protocol {
	protocols := make([]p2p.Protocol, 0, len(ProtocolVersions))
	for _, version := range ProtocolVersions {
		// Blob transactions require x7c/68 announcements, disable everything else
...
// NodeInfo represents a short summary of the `x7c` sub-protocol metadata
...
// nodeInfo retrieves some `x7c` protocol metadata about the running host node.
...
// Handle is invoked whenever an `x7c` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `x7c`", "err", err)
			return err
		}
	}
}
