...
func (x7c *Ethereum) hashState(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	var (
		current  *types.Block
	...
	if readOnly {
		...
		if statedb, err = x7c.blockchain.StateAt(block.Root()); err == nil {
			x7c.blockchain.TrieDB().Reference(block.Root(), common.Hash{})
			return statedb, func() {
				x7c.blockchain.TrieDB().Dereference(block.Root())
			}, nil
		}
	}
	...
	if base != nil {
		if preferDisk {
			...
			database = state.NewDatabaseWithConfig(x7c.chainDb, trie.HashDefaults)
		...
		current = x7c.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	} else {
		...
		// TODO(rjl493456442), clean cache is disabled to prevent memory leak,
		// please re-enable it for better performance.
		triedb = trie.NewDatabase(x7c.chainDb, trie.HashDefaults)
		database = state.NewDatabaseWithNodeDB(x7c.chainDb, triedb)
		...
		for i := uint64(0); i < reexec; i++ {
			...
			parent := x7c.blockchain.GetBlock(current.ParentHash(), current.NumberU64()-1)
			if parent == nil {
	...
	for current.NumberU64() < origin {
		...
		if current = x7c.blockchain.GetBlockByNumber(next); current == nil {
			return nil, nil, fmt.Errorf("block #%d not found", next)
		}
		_, _, _, err := x7c.blockchain.Processor().Process(current, statedb, vm.Config{})
		if err != nil {
			return nil, nil, fmt.Errorf("processing block %d failed: %v", current.NumberU64(), err)
		}
		// Finalize the state so any modifications are written to the trie
		root, err := statedb.Commit(current.NumberU64(), x7c.blockchain.Config().IsEIP158(current.Number()))
		if err != nil {
...
func (x7c *Ethereum) pathState(block *types.Block) (*state.StateDB, func(), error) {
	// Check if the requested state is available in the live chain.
	statedb, err := x7c.blockchain.StateAt(block.Root())
	if err == nil {
		return statedb, noopReleaser, nil
	}
...
//     on disk.
func (x7c *Ethereum) stateAtBlock(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	if x7c.blockchain.TrieDB().Scheme() == rawdb.HashScheme {
		return x7c.hashState(ctx, block, reexec, base, readOnly, preferDisk)
	}
	return x7c.pathState(block)
}

// stateAtTransaction returns the execution environment of a certain transaction.
func (x7c *Ethereum) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64) (*core.Message, vm.BlockContext, *state.StateDB, tracers.StateReleaseFunc, error) {
	// Short circuit if it's genesis block.
	if block.NumberU64() == 0 {
		return nil, vm.BlockContext{}, nil, nil, errors.New("no transaction in genesis")
	}
	// Create the parent state database
	parent := x7c.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.BlockContext{}, nil, nil, fmt.Errorf("parent %#x not found", block.ParentHash())
	}
	// Lookup the statedb of parent block from the live database,
	// otherwise regenerate it on the flight.
	statedb, release, err := x7c.stateAtBlock(ctx, parent, reexec, nil, true, false)
	if err != nil {
	...
	signer := types.MakeSigner(x7c.blockchain.Config(), block.Number(), block.Time())
	for idx, tx := range block.Transactions() {
		// Assemble the transaction call message and return if the requested offset
		msg, _ := core.TransactionToMessage(tx, signer, block.BaseFee())
		txContext := core.NewEVMTxContext(msg)
		context := core.NewEVMBlockContext(block.Header(), x7c.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, release, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, txContext, statedb, x7c.blockchain.Config(), vm.Config{})
