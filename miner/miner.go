...
func New(x7c Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(header *types.Header) bool) *Miner {
	miner := &Miner{
		mux:     mux,
		x7c:     x7c,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, x7c, mux, isLocalBlock, true),
	}
	miner.wg.Add(1)
	go miner.update()
	return miner
