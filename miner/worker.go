...
import (
	...
	"github.com/X7-Group/x7chain/consensus/misc/eip1559"
...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, x7c Backend, mux *event.TypeMux, isLocalBlock func(header *types.Header) bool, init bool) *worker {
	worker := &worker{
		config:             config,
	...
	worker.txsSub = x7c.TxPool().SubscribeTransactions(worker.txsCh, true)
	// Subscribe events for blockchain
	worker.chainHeadSub = x7c.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
			...
			if !w.isRunning() && w.current != nil {
				...
				for _, tx := range ev.Txs {
					...
					txs[acc] = append(txs[acc], &txpool.LazyTransaction{
						Pool:      w.x7c.TxPool(), // We don't know where this came from, yolo resolve from everywhere
						Hash:      tx.Hash(),
...
func (w *worker) fillTransactions(interrupt *atomic.Int32, env *environment) error {
	pending := w.x7c.TxPool().Pending(true)

	// Split the pending transactions into locals and remotes.
	localTxs, remoteTxs := make(map[common.Address][]*txpool.LazyTransaction), pending
	for _, account := range w.x7c.TxPool().Locals() {
...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		// https://github.com/X7-Group/x7chain/issues/24299
		env := env.copy()
